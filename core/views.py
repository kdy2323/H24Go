...
from .models import Payment, Proposition, User, Voiture, Taxi, Client, Course, Coiffeuse, DisponibiliteCoiffeuse, DemandeWebsite
...
def taxi_profile(request):
    ...
    if request.method == 'POST':
        ...
        taxi.iban = request.POST.get('iban', '').strip().replace(' ', '')  # ← ajoutez cette ligne
...
@login_required
def annuler_acceptation(request, course_id):
    """Remet la course en pending si le paiement n'a pas abouti."""
    client_user = request.user
    course = get_object_or_404(Course, id=course_id, client__user=client_user)
 
    if request.method == "POST" and course.status == "accepted":
        # Supprimer le paiement pending lié
        Payment.objects.filter(course=course, status="pending").delete()
 
        # Remettre la course en attente
        course.status = "pending"
        course.taxi = None
        course.prix_propose = None
        course.temps_arrivee = None
        course.save()
...
def client_courses_valides(request):
    client = request.user.client
    courses = Course.objects.filter(client=client, status="paid").select_related("taxi").order_by("-created_at")
...
def client_search_coiffeuse(request):
    ...
    if ville:
        # Coiffeuses avec disponibilités enregistrées
        avec_dispos = DisponibiliteCoiffeuse.objects.filter(
            actif=True
        ).values_list('coiffeuse_id', flat=True)

        coiffeuses = Coiffeuse.objects.filter(
            ville__iexact=ville
        ).filter(
            Q(is_available=True) | Q(id__in=avec_dispos)
        ).distinct()
...
@login_required
def ajax_propositions(request):
    client = request.user.client
    courses = Course.objects.filter(
        client=client,
        status__in=["pending", "accepted"]
    ).prefetch_related("propositions__taxi__user")

    data = []
    for course in courses:
        props = []
        for p in course.propositions.all():
            props.append({
                "id": p.id,
                "taxi": p.taxi.user.username,
        ...
        data.append({
            ...
            "arrivee": course.adresse_arrivee,
            "status": course.status,
            "taxi": course.taxi.user.username if course.taxi else None,
            "prix_propose": str(course.prix_propose) if course.prix_propose else None,
            "temps_arrivee": course.temps_arrivee,
            "propositions": props,
        })
...
def admin_dashboard(request):
    ...
    nb_taxis = taxis.count()
    nb_coiffeuses = Coiffeuse.objects.count()
    demandes_website = DemandeWebsite.objects.filter(traitee=False).select_related('coiffeuse__user')
    
    # Inscriptions payées taxis
    nb_taxis_payes = sum(
        1 for t in taxis
        if Payment.objects.filter(user=t.user, role="taxi", status="success").exists()
    )
    nb_taxis_non_payes = nb_taxis - nb_taxis_payes

    # Inscriptions payées coiffeuses
    coiffeuses = Coiffeuse.objects.all()
    nb_coiffeuses_payees = sum(
        1 for c in coiffeuses
        if Payment.objects.filter(user=c.user, role="coiffeuse", status="success").exists()
    )
    nb_coiffeuses_non_payees = nb_coiffeuses - nb_coiffeuses_payees

    # Soldes et courses par taxi
    for taxi in taxis:
        ...
        taxi.courses_recues = courses_paid.filter(paiement_admin=True)

    return render(request, "admin/dashboard.html", {
        "taxis": taxis,
        "nb_clients": nb_clients,
        "nb_taxis": nb_taxis,
        "nb_coiffeuses": nb_coiffeuses,
        "nb_taxis_payes": nb_taxis_payes,
        "nb_taxis_non_payes": nb_taxis_non_payes,
        "nb_coiffeuses_payees": nb_coiffeuses_payees,
        "nb_coiffeuses_non_payees": nb_coiffeuses_non_payees,
        "demandes_website": demandes_website,
    })


@staff_member_required
def traiter_demande_website(request, demande_id):
    demande = get_object_or_404(DemandeWebsite, id=demande_id)
    demande.traitee = True
    demande.save()
    return redirect('admin_dashboard')

@staff_member_required
def supprimer_demande_website(request, demande_id):
    demande = get_object_or_404(DemandeWebsite, id=demande_id)
    demande.delete()
    return redirect('admin_dashboard')
...
def coiffeuse_disponibilites(request):
    ...
    if request.method == 'POST':
        for jour in jours:
            actif = request.POST.get(f'{jour}_actif') == 'on'
            debut = request.POST.get(f'{jour}_debut') or '09:00'
            fin = request.POST.get(f'{jour}_fin') or '17:00'
            DisponibiliteCoiffeuse.objects.update_or_create(
                coiffeuse=coiffeuse, jour=jour,
                defaults={'heure_debut': debut, 'heure_fin': fin, 'actif': actif}
            )
        messages.success(request, "Disponibilités mises à jour !")
        return redirect('coiffeuse_disponibilites')

    dispos = {d.jour: d for d in DisponibiliteCoiffeuse.objects.filter(coiffeuse=coiffeuse)}
    return render(request, 'coiffeuse/coiffeuse_disponibilites.html', {
        'coiffeuse': coiffeuse, 'jours': jours, 'dispos': dispos
    })
...
def coiffeuse_prestations(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Créer la demande seulement si vient de cocher
            if not ancien_wants_website and coiffeuse.wants_website:
                DemandeWebsite.objects.get_or_create(coiffeuse=coiffeuse)

            # Supprimer la demande si décoché
            elif ancien_wants_website and not coiffeuse.wants_website:
                DemandeWebsite.objects.filter(coiffeuse=coiffeuse).delete()
...
def manifest_json(request):
    data = {
        "name": "H24Go",
        "short_name": "H24Go",
        "description": "Votre plateforme pour client, coiffeuse et taxi en Belgique",
        "start_url": "/",
        "display": "standalone",
        "background_color": "#201c1b",
        "theme_color": "#eecb04",
...
def service_worker(request):
    ...
    sw_path = os.path.join(settings.BASE_DIR, 'core', 'static', 'js', 'service-worker.js')
